...
func TestRegistrySlackRowsArePageSpecific(t *testing.T) {
	for _, tt := range []struct {
		...
		{name: "commands", page: PageCommands, want: []string{"read:31", "arg verbs", "mutation surface"}},
...
func TestTitleBarRendersChannelHotlistWithActivity(t *testing.T) {
	...
	for _, want := range []string{"REINS", "win", "1.events:", "2.tasks:2!1", "‹3.sess:1!1›", "Y.yard:2!1", "R.ready:1!0", "I.obs", "C.caps:18c!12", "4.dyn:1@all", "A.loops:0L", "X.axes:6A", ".ident:4I", "E.epi", "5.help:ref", "B.consent:4A6", "6.cmds:35", "7.wins:25", "8.intent:9", "9.surf:35", "0.doms:17", "L.life:6", "+3›"} {
...
func TestCommandAndWindowCatalogPagesRender(t *testing.T) {
	...
	cmdView = ansi.Strip(cmds.View())
	for _, want := range []string{"tasks (t)", "read/window", "selected    tasks (t)", "[j/k]command 2/35"} {
		if !strings.Contains(cmdView, want) {
