...
func TestRegistrySlackRowsArePageSpecific(t *testing.T) {
	for _, tt := range []struct {
		...
		{name: "commands", page: PageCommands, want: []string{"read:30", "arg verbs", "mutation surface"}},
...
func TestTitleBarRendersChannelHotlistWithActivity(t *testing.T) {
	...
	for _, want := range []string{"REINS", "win", "1.events:", "2.tasks:2!1", "‹3.sess:1!1›", "Y.yard:2!1", "R.ready:1!0", "I.obs", "C.caps:18c!12", "4.dyn:1@all", "A.loops:0L", "X.axes:6A", ".ident:4I", "E.epi", "5.help:ref", "B.consent:4A6", "6.cmds:34", "7.wins:25", "8.intent:9", "9.surf:35", "0.doms:17", "L.life:6", "+3›"} {
...
func TestWindowRegistrySeparatesEngineAndInstanceLifecycleWindows(t *testing.T) {
	...
	if !strings.Contains(out.Status, "windows:") || !strings.Contains(out.Status, "engine 8") || !strings.Contains(out.Status, "instance 17") || !strings.Contains(out.Status, "intake") || !strings.Contains(out.Status, "routing") || !strings.Contains(out.Status, "sdlc") {
...
func TestExecHotkeyAndCycleOpenRegisteredPages(t *testing.T) {
	...
	if m.Page != PageSessionTurns {
		t.Fatalf("] must cycle to session turns after dispatch, got page %d", m.Page)
	}
	m = step(m, "]")
	if m.Page != PageLegend {
		t.Fatalf("] must cycle to legend after session turns, got page %d", m.Page)
	}
	m = step(m, "]")
	...
	m = step(m, "[")
	if m.Page != PageSessionTurns {
		t.Fatalf("[ must cycle from legend to session turns, got page %d", m.Page)
	}
	m = step(m, "[")
	if m.Page != PageDispatch {
		t.Fatalf("[ must cycle from session turns to dispatch, got page %d", m.Page)
	}
	m = step(m, "[")
	if m.Page != PageTraces {
...
func TestCommandAndWindowCatalogPagesRender(t *testing.T) {
	m := New("REINS")
	m.Width, m.Height = 260, 80 // tall enough to render the full catalog (32 verbs incl. the trailing govern verbs)
	...
	cmdView = ansi.Strip(cmds.View())
	for _, want := range []string{"tasks (t)", "read/window", "selected    tasks (t)", "[j/k]command 2/34"} {
		if !strings.Contains(cmdView, want) {
	...
	for _, want := range []string{"WINDOWS", "sessions", "yard", "readiness", "intake", "triage", "routing", "matrix", "sdlc", "cockpit", "gate", "axes", "identity", "relational", "consent", "epistemics", "commands", "surfaces", "domains", "lifecycles", "PageYard", "PageReadiness", "PageIntake", "PageCaps", "PageAxes", "PageIdentity", "PageRelational", "PageEpistemics", "PageCommands", "PageIntent", "PageSurfaces", "PageDomains", "PageLifecycles", "SPLIT", "link:compact role", "link:target actor", "link:scroll role", "anchor:target system", "anchor:ref framework", "anchor:ref who-is-acting", "anchor:ref who-is-affected", "anchor:target evidence", "anchor:ref tenant", "source-only", "selected    tasks", "[j/k]window 2/25", "jump        [2] / :tasks"} {
