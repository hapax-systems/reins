...
_KIND_SEVERITY = {  # kind -> base severity in [0,1]; escalations rank above routine
    ...
    "coord_dispatch.launch_failed": 0.7,
    "coord_dispatch.launch_succeeded": 0.5,
    "coord_dispatch.launch_started": 0.4,
    "sdlc.stage_transition": 0.5,
    "sdlc.authorization_flip": 0.65,
    "task.claim": 0.45,
}
...
def to_event(raw: dict, allowlist: list[str], age_s: float) -> dict:
    ...
    fields = {
        "ts": str(raw.get("timestamp", raw.get("ts", ""))),
        "kind": kind,
...
def _doc_source_ref_labels(value: Any, limit: int = 8) -> list[str]:
    ...
    for item in value:
        ...
        if isinstance(item, dict):
            raw_refs = (
                item.get("source_refs") or item.get("evidence_refs") or item.get("refs")
            )
...
def _orchestration_ledger_dir(cfg: dict | None = None) -> Path:
    configured = os.environ.get("REINS_ORCHESTRATION_LEDGER_DIR") or str(
        (cfg or {}).get("orchestration_ledger_dir") or ""
    )
    if configured:
...
def _route_id_from_record(rec: dict) -> str:
    route_id = str(
        rec.get("route_id")
        or rec.get("selected_route_id")
        or rec.get("dimensional_selected_route_id")
        or ""
    )
    if route_id:
        return route_id
...
def _put_latest_binding(
    bindings: dict[tuple[str, str], dict], key: tuple[str, str], rec: dict
) -> None:
    prior = bindings.get(key)
    if prior is not None and str(rec.get("created_at") or "") < str(
...
def _put_receipt_binding(
    bindings: dict[tuple[str, str], dict], key: tuple[str, str], rec: dict
) -> None:
    prior = bindings.get(key)
    if (
        prior is not None
        and str(prior.get("source_name") or "") == "methodology-dispatch.jsonl"
    ):
...
def _prefix_binding(
    role: str, claimed_task: str, bindings: dict[tuple[str, str], dict]
) -> dict | None:
    ...
    candidates.sort(
        key=lambda rec: (
            1
            if str(rec.get("source_name") or "") == "methodology-dispatch.jsonl"
            else 0,
...
def _route_decision_state(rec: dict) -> str:
    action = str(
        rec.get("action") or rec.get("decision") or rec.get("route_policy_action") or ""
    )
...
def _route_binding_index(
    cfg: dict | None = None,
) -> tuple[dict[tuple[str, str], dict], str, Path]:
    ...
    for rec in _iter_jsonl_tail_bytes(methodology_path):
        ...
        binding = {
            ...
            "profile": str(
                rec.get("profile") or (route_decision or {}).get("profile") or ""
            ),
            "platform": str(
                rec.get("platform") or (route_decision or {}).get("platform") or ""
            ),
            "created_at": created_at,
            "decision_id": decision_id,
        ...
        if route_decision:
            ...
            if prior is None or (
                (
                    str(prior.get("platform") or "") == ""
                    or str(prior.get("platform") or "") == binding["platform"]
                )
                and (
                    str(prior.get("route_id") or "") == ""
                    or str(prior.get("route_id") or "") == binding["route_id"]
                )
            ):
                _put_receipt_binding(bindings, (lane, task_id), binding)
...
def _session_route_binding(
    ...
    if (
        ...
        and str(lane.get("platform") or "")
        and str(rec.get("platform")) != str(lane.get("platform"))
    ):
        base["route_binding_state"] = "platform_mismatch"
        base["route_evidence_ref"] = _route_binding_ref(
            rec, str(rec.get("source_name") or source_path.name)
        )
        return base
    return {
        ...
        "route_evidence_ref": _route_binding_ref(
            rec, str(rec.get("source_name") or source_path.name)
        ),
    }
...
def to_session(
    name: str, lane: dict, allowlist: list[str], route_binding: dict | None = None
) -> dict:
    ...
    fields = {
        ...
        "attention": _session_attention(
            lane, state, readiness, blocker, output_age_s, relay_age_s
        ),
    }
    if route_binding:
...
def _session_state_path() -> str:
    return os.path.expanduser(
        os.environ.get(
            "REINS_COORDINATOR_STATE", "/dev/shm/hapax-coordinator/state.json"
        )
    )
...
def _evidence_ref(
    kind: str, path: Path, privacy: str, raw_access: bool = False
) -> dict:
    try:
        st = path.stat()
        size = int(st.st_size)
...
def _intake_source(
    source_id: str, path: Path, count: int, status: str, allowlist: list[str]
) -> dict:
    fields = {
        "id": source_id,
        "path": str(path),
...
def _intake_missing(
    status: str, severity: str, blocker: str, task_link_state: str, count: int
) -> str:
    parts: list[str] = []
    ...
    if task_link_state and task_link_state not in {
        "task_visible",
        "task_link_metadata",
        "task_metadata",
    }:
...
def _intake_paths(cfg: dict) -> dict[str, Path]:
    ...
    return {
        "request_state": cfg_path(
            "request_intake_state",
            "REINS_REQUEST_INTAKE_STATE",
            cache / "request-intake-state.json",
        ),
        "planning_feed": cfg_path(
            "planning_feed_state",
            "REINS_PLANNING_FEED_STATE",
            cache / "planning-feed-state.json",
        ),
        "p0_incident_state": cfg_path(
            "p0_incident_state",
            "REINS_P0_INCIDENT_STATE",
            cache / "p0-incident-intake" / "state.json",
        ),
        "p0_incident_ledger": cfg_path(
            "p0_incident_events",
            "REINS_P0_INCIDENT_EVENTS",
            cache / "p0-incident-intake" / "events.jsonl",
        ),
        "security_signal_state": cfg_path(
            "security_signal_state",
            "REINS_SECURITY_SIGNAL_STATE",
            cache / "security-signal-intake-state.json",
        ),
    }
...
def read_intake_summary(cfg: dict, allowlist: list[str]) -> dict:
    ...
    if p.exists():
        ...
        rows.append(
            _intake_row(
                ...
                "request_attention",
                "attention",
                "warn" if attention else "ok",
                attention,
                allowlist,
                coverage="workflow_attention",
                age_bucket=_path_age_bucket(p),
            )
        )
        if malformed:
            rows.append(
    ...
    if p.exists():
        ...
        attention = (
            len(data.get("attention_required") or [])
            if isinstance(data.get("attention_required"), list)
            else _safe_int(data.get("attention_required"))
        )
    ...
    if p.exists():
        ...
        sources.append(
            _intake_source(
                "p0_incident_state", p, len(incidents), "observed", allowlist
            )
        )
        ...
        for kind, n in sorted(by_kind.items(), key=lambda kv: (-kv[1], kv[0]))[:10]:
            rows.append(
                _intake_row(
                    ...
                    blocker="p0_incident",
                    coverage="coalesced",
                    task_link_state="task_link_metadata",
                    age_bucket=_path_age_bucket(p),
                )
            )
    else:
    ...
    if p.exists():
        ...
        sources.append(
            _intake_source("p0_incident_ledger", p, len(ledger), "observed", allowlist)
        )
    ...
    if p.exists():
        ...
        sources.append(
            _intake_source("security_signal_state", p, total, "observed", allowlist)
        )
        ...
        for kind, n in sorted(by_kind.items(), key=lambda kv: (-kv[1], kv[0]))[:6]:
            rows.append(
                _intake_row(
                    ...
                    n,
                    allowlist,
                    blocker="security_signal",
                    coverage="snapshot",
                    task_link_state="request_metadata",
                    age_bucket=_path_age_bucket(p),
                )
            )
        ...
        sources.append(
            _intake_source("security_signal_state", p, 0, "missing", allowlist)
        )
...
def _hkp_support_summary(
    cfg: dict, allowlist: list[str]
    ...
    index_root = (
        Path(os.path.expanduser(index_root_raw)) if index_root_raw else Path("")
    )
    report_root = (
        Path(os.path.expanduser(report_root_raw)) if report_root_raw else Path("")
    )
    ...
    for bundle in bundles:
        ...
        denied = {
            name for name, default in policy_defaults.items() if default == "deny"
        }
        safe_policy = (
            cache_only
            and {"research_viewer", "local_prompt_context"}.issubset(allowed)
            and required_denials.issubset(forbidden | denied)
        )
        ...
        index_path = (
            index_root / f"{safe_bundle}.jsonl" if str(index_root) else Path("")
        )
        index_count = _count_jsonl_lines(index_path) if str(index_path) else 0
        ref_count = max(
            index_count, concept_count + edge_count, event_count + edge_ledger_count
        )
        ...
        status = (
            "support-only" if safe_policy and manifest_path.exists() else "read-missing"
        )
        generated = str(
            snapshot.get("generated_at") or manifest.get("generated_at") or ""
        )
        detail = "cache-only HKP bundle; concepts=%d edges=%d generated=%s" % (
            concept_count,
            edge_count,
    ...
    if str(report_root):
        try:
            ...
            report_dirs = []
        newest = max(
            (p / "report.json" for p in report_dirs),
    ...
    blocker = (
        "; ".join(blockers)
        if blockers
        else "support-only cache/context; not source truth, dispatch authority, calibration evidence, release authority, provider spend, or public export"
    )
...
def _capability_route(route: Any, freshness: Any | None, allowlist: list[str]) -> dict:
    ...
    errors = (
        list(getattr(freshness, "errors", ()) or []) if freshness is not None else []
    )
    blocked_reasons = list(getattr(route, "blocked_reasons", ()) or [])
    freshness_ok = (
        bool(getattr(freshness, "ok", False)) if freshness is not None else False
    )
    evidence_count = len(getattr(freshness, "evidence_refs", ()) or [])
    quota_source = str(
        getattr(getattr(route, "telemetry", None), "quota_source", "unknown")
    )
    if "." in quota_source:
    ...
    fields = {
        ...
        "context_mode": _route_axis(
            route, desc, ["context_mode", "context_window", "context_budget"]
        ),
        "fast_mode": _route_axis(
            route, desc, ["fast_mode", "fast_path", "low_latency"]
        ),
        "quantization": _route_axis(
            route, desc, ["quantization", "quant", "precision"]
        ),
        "capacity_pool": _route_axis(
            route, desc, ["capacity_pool", "pool", "capacity"]
        ),
        "demand_vector": _route_axis(
            route, desc, ["demand_vector", "demand", "task_shape"]
        ),
        "hardening": _route_axis(
            route, desc, ["hardening", "hardening_intensity", "request_hardening"]
        ),
        "eval_plane": _route_axis(
            route, desc, ["eval_plane", "eval", "evaluation_plane"]
        ),
        "review_obligation": _route_axis(
            route, desc, ["review_obligation", "review", "reviewer_obligation"]
        ),
        "learning_eligibility": _route_axis(
            route, desc, ["learning_eligibility", "learning", "calibration_eligibility"]
        ),
        "benchmark_coverage": _route_axis(
            route, desc, ["benchmark_coverage", "benchmark", "coverage"]
        ),
        "fixed_overhead": _route_axis(
            route, desc, ["fixed_overhead", "overhead", "setup_cost"]
        ),
        "route_state": state,
        "authority_ceiling": str(
            getattr(
                getattr(route, "authority_ceiling", ""),
                "value",
                getattr(route, "authority_ceiling", ""),
            )
            or ""
...
def _route_axis(
    route: Any, desc: Any, fields: list[str], default: str = "missing"
) -> str:
    for field in fields:
...
_CAPABILITY_SURFACE_TAXONOMY: dict[str, tuple[str, str, str, str, str]] = {
    ...
    "google_drive_docs_connector": (
        ...
        "connector",
        "workspace_connector",
        "connector authority + egress receipt",
    ),
    "github_repo_ci": (
        "verifier_floor_checker",
        "github_repo_ci",
        "connector",
        "repo_ci",
        "governed task/review/queue receipt",
    ),
    ...
    "mistral_vibe_worker_surface": (
        ...
        "subscription",
        "coding_cli_session",
        "route envelope + scope receipt",
    ),
    "glmcp_review_quota_admission": (
        "verifier_floor_checker",
        "review_quota_admission",
        "api_or_subscription",
        "review_quota_signal",
        "review/quota/admission receipt",
    ),
    ...
    "glm_coding_plan_tool_surface": (
        ...
        "subscription",
        "tool_runtime",
        "manual bakeoff/admission receipt",
    ),
    "fugu_raw_codex": (
        "subscription_tool_surface",
        "sakana_fugu",
        "subscription",
        "coding_cli_session",
        "governed route + admission + bakeoff receipt",
    ),
    "fugu_ultra_raw_codex": (
        ...
        "subscription",
        "coding_cli_session",
        "governed route + admission + clean-host preflight + bakeoff receipt",
    ),
    ...
    "cohere_embed_rerank": (
        ...
        "api_spend",
        "retrieval_api",
        "model/pricing/use-case receipt",
    ),
    "litellm_provider_gateway": (
        ...
        "gateway",
        "provider_api",
        "route envelope; no silent fallback",
    ),
    "openrouter_break_glass": (
        "provider_gateway",
        "openrouter",
        "break_glass_api_spend",
        "provider_api",
        "emergency spend receipt",
    ),
    ...
    "picovoice_audio_verifier": (
        ...
        "api_or_local",
        "audio_runtime",
        "audio/runtime receipt",
    ),
    "audio_fingerprint_source_id": (
        "audio_avsdlc_tool",
        "audio_fingerprint",
        "api_spend",
        "media_external_query",
        "media privacy + egress receipt",
    ),
    ...
    "research_publication_deposit": (
        ...
        "external_account",
        "public",
        "publication + legal-name receipt",
    ),
    "public_social_distribution": (
        ...
        "external_account",
        "public",
        "publication + operator receipt",
    ),
    "google_workspace_youtube_connector": (
        "publication_egress",
        "google_workspace_youtube",
        "connector",
        "public_or_workspace",
        "connector + public-egress receipt",
    ),
    "research_storage_infra": (
        ...
        "infra_account",
        "infrastructure",
        "destructive preflight + infra receipt",
    ),
...
def _capability_taxonomy(capability_id: str) -> dict[str, str]:
    ...
    if capability_id in _CAPABILITY_SURFACE_TAXONOMY:
        capability_class, family, spend, egress, receipt = _CAPABILITY_SURFACE_TAXONOMY[
            capability_id
        ]
        return {
...
def _capability_pack_row(
    row: dict, source_id: str, allowlist: list[str], row_kind: str = "surface"
) -> dict:
    capability_id = str(
        row.get("capability_id") or row.get("id") or row.get("name") or ""
    )
    out = _capability_row(
        capability_id,
        str(row.get("status") or row.get("state") or "read-missing"),
        str(
            row.get("routing_meaning")
            or row.get("evidence_posture")
            or row.get("authority")
            or ""
        ),
        _safe_int(
            row.get("route_count") or row.get("surface_count") or row.get("count")
        ),
        _safe_int(
            row.get("ok_count") or row.get("ready_count") or row.get("observed_count")
        ),
        _safe_int(
            row.get("blocked_count") or row.get("gap_count") or row.get("missing_count")
        ),
...
def _compiled_capability_surface_rows(allowlist: list[str]) -> list[dict]:
    ...
    for row in rows:
        ...
        row["air"] = classify_air(
            {k: v for k, v in row.items() if k != "air"}, allowlist
        )
    return rows
...
def _capability_inventory_path(cfg: dict | None = None) -> Path | None:
    ...
    cfg = cfg or {}
    raw = (
        os.environ.get("REINS_CAPABILITY_INVENTORY")
        or cfg.get("capability_inventory_path")
        or ""
    )
    raw = str(raw).strip()
...
def _capability_inventory_rows(
    cfg: dict | None, allowlist: list[str]
    ...
    for r in raw_rows:
        ...
        gaps = [
            str(g)
            for g in (raw_gaps if isinstance(raw_gaps, (list, tuple)) else [raw_gaps])
            if str(g).strip()
        ]
        ...
        for key, val in (
            ...
            if val:
                out[key] = str(val)
        out["air"] = classify_air(
            {k: v for k, v in out.items() if k != "air"}, allowlist
        )
        rows.append(out)
    ...
    dark_n = (
        _safe_int(fresh_counts.get("dark")) if isinstance(fresh_counts, dict) else 0
    )
    detail = (
    ...
    return [
        _capability_source(
            "capability_inventory", path, total, "observed", allowlist, detail
        )
    ], rows


def _capability_surface_pack_rows(
    cfg: dict | None, allowlist: list[str]
) -> tuple[list[dict], list[dict]]:
    ...
    for path in paths:
        ...
        if not path.exists():
            sources.append(
                _capability_source(
                    ...
                    "missing",
                    allowlist,
                    "configured capability-surface source absent",
                )
            )
            continue
        try:
        ...
        pack_rows = [
            r for r in [*class_rows, *surface_rows, *extra_rows] if isinstance(r, dict)
        ]
        ...
        sources.append(
            _capability_source(pack_id, path, len(pack_rows), status, allowlist, detail)
        )
...
def read_capability_summary(
    council_root: str, allowlist: list[str], cfg: dict | None = None
) -> dict:
    ...
    registry_path = (
        Path(os.path.expanduser(council_root))
        / "config"
        / "platform-capability-registry.json"
    )

    receipt_dir = Path(
        os.path.expanduser(
            str(
                cfg.get("capability_receipt_dir")
    ...
    quota_path = Path(
        os.path.expanduser(
            str(cfg.get("quota_spend_ledger_live") or DEFAULT_QUOTA_SPEND_LEDGER_LIVE)
        )
    )
    sources_obj = load_dispatch_policy_sources(
        receipt_dir=receipt_dir, registry_path=registry_path
    )
    ...
    freshness_by_route = {
        r.route_id: r for r in (getattr(freshness, "routes", ()) or [])
    }
    ...
    if inv_rows:
        ...
        pack_sources = (
            inv_sources + pack_sources
        )  # keep the inventory-absent marker honest (DARK visibility)
    hkp_sources, hkp_evidence_count, hkp_source_refs, hkp_blocker = (
        _hkp_support_summary(cfg, allowlist)
    )
    sources = (
        [
            _capability_source(
            ...
            _capability_source(
                "platform_receipts",
                receipt_dir,
                _receipt_count(receipt_dir),
                "observed" if receipt_dir.exists() else "missing",
                allowlist,
                "local platform capability receipts",
            ),
            _capability_source(
                "route_authority_receipts",
                route_auth_dir,
                len(sources_obj.route_authority_receipts),
                "observed" if route_auth_dir.exists() else "missing",
                allowlist,
                "fresh route authority receipts",
            ),
            _capability_source(
                "quota_ledger",
                quota_path,
    ...
    route_rows = [
        _capability_route(route, freshness_by_route.get(route.route_id), allowlist)
        for route in routes
    ]
    tool_rows = [
        tool for route in routes for tool in _capability_tool_rows(route, allowlist)
    ]
    blocked_routes = sum(
        1 for r in route_rows if r["route_state"] == "blocked" or r["blockers"]
    )
    ...
    quota_route_ok = sum(
        1
        for route in routes
        if str(getattr(getattr(route, "telemetry", None), "quota_source", "")).rsplit(
            ".", 1
        )[-1]
        not in {"none", "unknown", ""}
    )
    ...
    rows.append(
        _capability_row(
            ...
            route_auth_count,
            "authority receipts are evidence, not dispatch grants"
            if route_auth_count
            else "route authority receipts absent",
            "not_applicable",
            allowlist,
        )
    )
    ...
    for route in routes:
        ...
        for dim, score in payload.items():
            bucket = score_dims.setdefault(
                dim, {"routes": 0, "ok": 0, "blocked": 0, "evidence": 0, "stale": 0}
            )
            bucket["routes"] += 1
    ...
    for dim, bucket in sorted(score_dims.items()):
        ...
        if bucket["evidence"] == 0:
            ...
            status, blocker = (
                "partial",
                f"{bucket['blocked']} routes missing score evidence",
            )
        rows.append(
            _capability_row(
                dim,
...
def read_gate_summary(
    ...
    try:
        ...
        sessions = [
            to_session(
                ...
                _session_route_binding(
                    name, lane, bindings, binding_source, binding_path
                ),
            )
        ...
        route_covered = sum(
            1
            for s in sessions
            if str(s.get("route_binding_state") or s.get("route_id") or "").strip()
        )
        sources.append(
            _gate_source(
                "route_binding",
    ...
    for gate_id, items in sorted(
        false_by_gate.items(), key=lambda x: (-len(x[1]), x[0])
    ):
        count = len(items)
    ...
    for blocker, count in sorted(blocker_counts.items(), key=lambda x: (-x[1], x[0])):
        rows.append(
            _gate_row(
                ...
                "coordinator_state",
                f"count={count}; readiness="
                + " ".join(
                    f"{k}:{readiness_counts[k]}" for k in sorted(readiness_counts)
                ),
                blocker,
                allowlist,
    ...
    for state, count in sorted(route_counts.items(), key=lambda x: (-x[1], x[0])):
        ...
        routes_part = " ".join(
            f"{route}:{route_evidence[route]}" for route in sorted(route_evidence)[:4]
        )
        evidence = f"count={count}"
        if routes_part:
    ...
    command_rows = [
        (
            ...
            "authority_case,parent_spec,preflight,receipt",
            ":intent dispatch",
        ),
        (
            "command.claim",
        ...
        (
            ...
            "preview-only",
            "crit",
            "operator/governed route",
            "release_authorized=true receipt",
            ":intent approve",
        ),
    ]
    ...
    blocked = sum(
        1 for row in rows if row.get("state") in {"blocked", "missing", "dark"}
    )
    preview = sum(
        1 for row in rows if row.get("state") in {"preview-only", "preview_only"}
    )
    totals.update(
        {
            "sources": len(sources),
...
def _domain_row(
    row: dict, source_id: str, pack_authority: str, allowlist: list[str]
) -> dict:
    ...
    fields = {
        "domain_id": domain_id,
        "label": str(
            row.get("label") or row.get("title") or row.get("name") or domain_id
        ),
        "lifecycle": str(
            row.get("lifecycle") or row.get("dlc") or row.get("domain") or ""
        ),
        ...
        "claim_ceiling": str(
            row.get("claim_ceiling") or row.get("claim") or "navigation"
        ),
        ...
        "source_refs": f"{source_id}:{evidence_count} refs"
        if evidence_count
        else source_id,
    }
    return {**fields, "air": classify_air(fields, allowlist)}
...
def _domain_relation(
    rel: dict, source_id: str, pack_authority: str, allowlist: list[str]
) -> dict:
    ...
    fields = {
        ...
        "authority_ceiling": str(
            rel.get("authority_ceiling")
            or rel.get("authority")
            or pack_authority
            or "metadata-only"
        ),
...
def _lifecycle_row(
    row: dict, source_id: str, registry_authority: str, allowlist: list[str]
) -> dict:
    lifecycle_id = str(
        row.get("lifecycle_id") or row.get("id") or row.get("name") or ""
    )
    evidence_count = _domain_evidence_count(row)
    fields = {
        "lifecycle_id": lifecycle_id,
        "label": str(
            row.get("label") or row.get("title") or row.get("name") or lifecycle_id
        ),
        ...
        "receipt_contracts": _csvish(
            row.get("receipt_contracts") or row.get("receipts")
        ),
        "evidence_count": evidence_count,
        "blocker": str(row.get("blocker") or ""),
...
def read_lifecycle_registry_summary(cfg: dict | None, allowlist: list[str]) -> dict:
    ...
    for path in paths:
        ...
        if not path.exists():
            sources.append(
                _domain_source(
                    ...
                    allowlist,
                    "compiled-fallback",
                    "configured lifecycle registry absent",
                )
            )
            continue
        try:
            ...
            continue
        registry_id = str(
            doc.get("registry_id") or doc.get("pack_id") or doc.get("id") or source_id
        )
        registry_authority = str(
            doc.get("authority_case") or doc.get("authority") or ""
        )
        ...
        if not default_lens:
            default_lens = str(
                doc.get("default_lens") or doc.get("default_projection") or ""
            )
        ...
        for row in lifecycle_rows:
            if isinstance(row, dict):
                rows.append(
                    _lifecycle_row(row, registry_id, registry_authority, allowlist)
                )
        ...
        sources.append(
            _domain_source(
                ...
                len(lifecycle_rows),
                allowlist,
                registry_authority or "metadata-only",
    ...
    if len(nonempty_hashes) == 1:
        ...
        package_hash = (
            "sha256:"
            + hashlib.sha256("|".join(nonempty_hashes).encode("utf-8")).hexdigest()
        )
...
def read_domain_pack_summary(cfg: dict | None, allowlist: list[str]) -> dict:
    ...
    for path in paths:
        ...
        for rel in relation_rows:
            if isinstance(rel, dict):
                relations.append(
                    _domain_relation(rel, pack_id, pack_authority, allowlist)
                )
        ...
        sources.append(
            _domain_source(
                ...
                path,
                status,
                len(domain_rows) + len(relation_rows),
                allowlist,
                pack_authority or "metadata-only",
    ...
    return {
        ...
        "authority": authority
        or ("source-backed" if rows or relations else "compiled-fallback"),
        "generated_at": generated_at,
        "package_hash": package_hash,
        "default_lens": default_lens,
        "lifecycle_sources": lifecycle["sources"],
...
def _transcript_refs(role: str, platform: str, cfg: dict) -> tuple[list[dict], dict]:
    ...
    for root in roots:
        ...
        for path in root.rglob("*"):
            ...
            refs.append(
                _evidence_ref(
                    "transcript_candidate", path, "raw_transcript", raw_access=False
                )
            )
    return refs, {
...
def to_session_detail(
    ...
    if note_path:
        refs.append(
            _evidence_ref("cc_task_note", note_path, "private", raw_access=False)
        )
    transcript_refs, evidence_scan = _transcript_refs(
        base["role"], base["platform"], cfg
    )
...
def read_vault_summary(cfg: dict | None = None) -> dict:
    ...
    return {
        "vault_root": str(root),
        "dark": False,
        "notes": [row for _, row in notes[:500]],
    }
...
def _observe_timeout_s(cfg: dict | None = None) -> float:
    cfg = cfg or {}
    raw = (
        os.environ.get("REINS_OBSERVE_TIMEOUT_S") or cfg.get("observe_timeout_s") or 0.2
    )
    try:
...
def _observe_session_dimensions() -> tuple[dict, dict]:
    ...
    idle = sum(
        1
        for _, lane in raw
        if lane.get("idle") and lane.get("alive") and not lane.get("stalled")
    )
...
def _observe_governance(cfg: dict | None) -> dict:
    ...
    try:
        ...
        for task in tasks.values():
            ...
            if isinstance(no_go, dict) and any(
                value is False for value in no_go.values()
            ):
                blocked += 1
        return _observe_dim(
            "governance",
...
def _dispatch_ledger_path(cfg: dict | None = None) -> Path:
    cfg = cfg or {}
    raw = os.environ.get("REINS_DISPATCH_LEDGER_PATH") or str(
        cfg.get("dispatch_ledger_path") or ""
    )
    if raw:
        return Path(os.path.expanduser(raw))
...
def _observe_cost(cfg: dict | None) -> dict:
    ...
    if council_root:
        ...
        return _observe_dark(
            "cost",
            str(traces.get("error") or "dispatch ledger missing and traces dark"),
        )
...
def _claim_rollups(claims: list[dict]) -> dict[tuple[str, str, str, str], int]:
    ...
    for claim in claims:
        key = (
            ...
            str(
                ((claim.get("provenance") or {}).get("authority_ceiling")) or "unknown"
            ),
...
def _workbench_scene(scene: Any, allowlist: list[str]) -> dict:
    if not isinstance(scene, dict):
        scene = {}
    selection = (
        scene.get("selection") if isinstance(scene.get("selection"), dict) else {}
    )
    fields = {
...
def _dynamics_workbench_contract(manifest: dict, allowlist: list[str]) -> dict:
    ...
    for row in raw.get("explanation_paths") or []:
        if not isinstance(row, dict):
            continue
        scenes = [
            _workbench_scene(scene, allowlist)
...
def read_dynamics_package(council_root: str, allowlist: list[str]) -> dict:
    ...
    claims = (
        claims_doc.get("claims") if isinstance(claims_doc.get("claims"), list) else []
    )
    lenses = _manifest_lenses(manifest, lenses_doc)
    observations = _read_jsonl(observations_path)
    relations = (
        relations_doc.get("relations")
        if isinstance(relations_doc.get("relations"), list)
        else []
    )
    ...
    sources = [
        _dyn_source(
            "package",
            "observed" if package_doc else "missing",
            len(package_doc),
            allowlist,
            "package metadata",
            _path_age_bucket(package_path),
            _basename(package_path),
        ),
        _dyn_source(
            "manifest",
            "observed" if manifest else "missing",
            len(manifest),
            allowlist,
            "view manifest/workbench",
            _path_age_bucket(manifest_path),
            _basename(manifest_path),
        ),
        _dyn_source(
            "lock",
            "observed" if lock else "missing",
            len(lock),
            allowlist,
            "hash lock/staleness policy",
            _path_age_bucket(lock_path),
            _basename(lock_path),
        ),
        _dyn_source(
            "claims",
            "observed" if claims else "missing",
            len(claims),
            allowlist,
            "claim fragments",
            _path_age_bucket(claims_path),
            _basename(claims_path),
        ),
        _dyn_source(
            "lenses",
            "observed" if lenses else "missing",
            len(lenses),
            allowlist,
            "declared view lenses",
            _path_age_bucket(lenses_path),
            _basename(lenses_path),
        ),
        _dyn_source(
            "observations",
            "observed" if observations else "missing",
            len(observations),
            allowlist,
            "temporal observation state",
            _path_age_bucket(observations_path),
            _basename(observations_path),
        ),
        _dyn_source(
            "relations",
            "observed" if relations else "missing",
            len(relations),
            allowlist,
            "relation vocabulary",
            _path_age_bucket(relations_path),
            _basename(relations_path),
        ),
        _dyn_source(
            "canonical_trig",
            "present" if trig_path.exists() else "missing",
            1 if trig_path.exists() else 0,
            allowlist,
            "canonical RDF graph; not parsed here",
            _path_age_bucket(trig_path),
            _basename(trig_path),
        ),
        _dyn_source(
            "shacl",
            "present" if shacl_path.exists() else "missing",
            1 if shacl_path.exists() else 0,
            allowlist,
            "validation shapes; not parsed here",
            _path_age_bucket(shacl_path),
            _basename(shacl_path),
        ),
    ]

    validation: list[dict] = []
    val_doc = (
        package_doc.get("validation")
        if isinstance(package_doc.get("validation"), dict)
        else {}
    )
    for key, command in sorted(val_doc.items()):
        validation.append(
            _dyn_row(
                ...
                1,
                allowlist,
                f"command_ref={_command_ref(command)}",
                "package",
                "info",
            )
        )
    ...
    for lens in lenses:
        agg = (
            lens.get("aggregation") if isinstance(lens.get("aggregation"), dict) else {}
        )
        count = _safe_int(
            lens.get("visible_node_count", len(lens.get("visible_node_ids") or []))
        )
        edges = _safe_int(
            lens.get("visible_edge_count", len(lens.get("visible_edge_ids") or []))
        )
        ...
        lens_rows.append(
            _dyn_row(
                ...
                detail,
                "lenses",
                "warn" if status == "lossy" else "info",
            )
        )
    ...
    claim_rows = [
        ...
        for (kind, claim_type, authority, freshness), count in sorted(
            _claim_rollups(claims).items()
        )
    ]
    obs_rows = [
        _dyn_row(
            ...
            "observations",
            "warn" if freshness == "stale" else "info",
        )
        for (state, source_type, freshness), count in sorted(
            _observation_rollups(observations).items()
        )
    ]
    rel_rows = [
        _dyn_row(
            "relation",
            f"{category}:{claim_types}",
            "declared",
            vals["relations"],
            allowlist,
            f"edge_count={vals['edges']}",
            "relations",
            "info",
        )
        for (category, claim_types), vals in sorted(
            _relation_rollups(relations).items()
        )
    ]

    source_snapshot = (
        manifest.get("source_snapshot")
        if isinstance(manifest.get("source_snapshot"), dict)
        else {}
    )
    totals = {
        "sources": len(sources),
    ...
    return {
        ...
        "totals": totals,
        "authority_case": str(
            package_doc.get("authority_case") or manifest.get("authority_case") or ""
        ),
        "generated_at": str(
            package_doc.get("generated_at")
            or manifest.get("generated_at")
            or lock.get("generated_at")
            or ""
        ),
        "package_hash": str(lock.get("package_hash") or ""),
        "default_lens": str(
            (lenses_doc.get("default_lens") if isinstance(lenses_doc, dict) else "")
            or manifest.get("default_projection")
            or ""
        ),
...
def _dynamics_claim_ref_index(claims: list[dict]) -> dict[tuple[str, str], list[str]]:
    ...
    for claim in claims:
        ...
        labels: list[str] = []
        provenance = (
            claim.get("provenance") if isinstance(claim.get("provenance"), dict) else {}
        )
        _append_unique_labels(labels, _field_ref_labels(provenance))
        _append_unique_labels(labels, _field_ref_labels(claim))
        kind = _map_kind_token(
            claim.get("element_kind")
            or claim.get("map_kind")
            or claim.get("subject_kind")
        )
...
def _dynamics_observation_ref_index(
    ...
    for obs in observations:
        if not isinstance(obs, dict):
            continue
        kind = _map_kind_token(
            obs.get("element_kind") or obs.get("map_kind") or obs.get("subject_kind")
        )
        _index_map_ref_labels(
            index, kind, _map_element_ids(obs), _observation_evidence_labels(obs)
        )
    return index
...
def _epistemics_package_row(
    row: dict, authority_case: str, freshness: str, allowlist: list[str]
) -> dict:
    ...
    posture = (
        "declared"
        if family == "validation" or status == "declared"
        else "source-backed"
    )
...
def _epistemics_package_row_id(
    family: str, item_id: str, status: str, detail: str
) -> str:
...
def read_epistemics(
    council_root: str, allowlist: list[str], scope: str = "dynamics"
) -> dict:
    if scope != "dynamics":
    ...
    for node in nodes:
        ...
        refs = _map_element_source_labels(
            "node",
            node_id,
            _doc_ref_labels(node.get("docs")),
            claim_refs,
            observation_refs,
        )
        rows.append(
    ...
    for family in ("validation", "lenses", "claims", "observations", "relations"):
        for package_row in package.get(family) or []:
            if isinstance(package_row, dict):
                package_rows.append(
                    _epistemics_package_row(
                        package_row, authority_case, freshness, allowlist
                    )
                )
    ...
    return {
        ...
        "totals": {
            ...
            "missing_sources": _safe_int(
                (package.get("totals") or {}).get("missing_sources")
            )
            + (0 if nodes or edges else 1),
        },
    }
...
def read_traces(
    council_root: str, limit: int = 40, allowlist: list[str] | None = None
) -> dict:
...
def _projection(council_root: str) -> dict:
    ...
    return CoordProjection.from_replay(
        default_event_log().replay(fail_open=True)
    ).to_record()
...
def _page_before(records: list[dict], before: str | None, limit: int) -> list[dict]:
    ...
    if before:
        records = [
            r for r in records if str(r.get("timestamp", r.get("ts", ""))) < before
        ]
...
def _load_context_payload_bytes(council_root: str) -> bytes | None:
    """Read the exclusive producer path without interpreting its payload."""
    ...
    paths = (
        ...
        if env
            ...
            os.path.join(
                os.path.expanduser(council_root or ""), "reins-context-bundle.json"
            )
        ]
    )
    for p in paths:
        ...
        try:
            ...
            if not stat.S_ISREG(os.fstat(fd).st_mode):
                raise reins_context.ContextReadError("producer_malformed")
            with os.fdopen(fd, "rb", closefd=False) as source:
                payload = source.read(reins_context.MAX_CONTEXT_READ_BYTES + 1)
            if len(payload) > reins_context.MAX_CONTEXT_READ_BYTES:
                raise reins_context.ContextReadError("producer_payload_too_large")
            return payload
        except FileNotFoundError:
            continue
        except reins_context.ContextReadError:
            raise
        except OSError as exc:
            raise reins_context.ContextReadError("producer_malformed") from exc
        finally:
            if fd >= 0:
                try:
                    os.close(fd)
...
def build_app(
    council_root: str, allowlist: list[str], session_cfg: dict | None = None
) -> FastAPI:
    app = FastAPI()
    ...
    def read_events(limit: int = 80, before: str | None = None) -> dict:
        ...
        events = [
            to_event(
                r,
                allowlist,
                age_s=_age_s(str(r.get("timestamp", r.get("ts", ""))), now),
            )
            for r in raws
        ]
    ...
    def read_dynamics() -> dict:
        ...
        layers = [
            {"id": str(L.get("id", "")), "label": str(L.get("label", ""))}
            for L in (seed.get("layers") or [])
        ]
        return {
            "dark": False,
    ...
    def read_session_detail(role: str) -> dict:
        try:
            ...
            bindings, binding_source, binding_path = _route_binding_index(session_cfg)
            route_binding = _session_route_binding(
                role, lane, bindings, binding_source, binding_path
            )
            return {
                "dark": False,
    ...
    def read_session_turns(
        role: str, before: str | None = None, limit: int = 80
    ) -> dict:
        try:
    ...
    def read_context(request: Request) -> Response:
        if request.scope.get("query_string", b""):
            body = reins_context.dark_readout_bytes("audience_unsupported")
            return Response(
                content=body,
        ...
        try:
            payload = _load_context_payload_bytes(council_root)
        except reins_context.ContextReadError as exc:
            body = reins_context.dark_readout_bytes(exc.reason_code)
        except Exception:
            body = reins_context.dark_readout_bytes("context_read_error")
        else:
            body = (
                reins_context.dark_readout_bytes("producer_absent")
                if payload is None
                else reins_context.parse_context_payload_bytes(payload)
            )
        return Response(
            content=body,
            media_type="application/json",
    ...
    def read_capabilities() -> dict:
        try:
            return {
                "dark": False,
                "capabilities": read_capability_summary(
...
def instance_config() -> dict:
    ...
    toml_cfg: dict = {}
    path = os.environ.get("REINS_CONFIG") or os.path.expanduser(
    ...
    council_root = os.environ.get("REINS_COUNCIL_ROOT") or str(
        toml_cfg.get("council_root", "")
    )
    cc_tasks_active = os.environ.get("REINS_CC_TASKS_ACTIVE") or str(
        toml_cfg.get("cc_tasks_active", "")
    )
    orchestration_ledger_dir = os.environ.get("REINS_ORCHESTRATION_LEDGER_DIR") or str(
        toml_cfg.get("orchestration_ledger_dir", "")
    )
    ...
    if lifecycle_env:
        ...
        if isinstance(raw_lifecycles, list):
            lifecycle_registry_paths = [
                str(v) for v in raw_lifecycles if str(v).strip()
            ]
        elif raw_lifecycles:
    ...
    if capability_surface_env:
        capability_surface_pack_paths = [
            v for v in capability_surface_env.split(os.pathsep) if v
        ]
    else:
        raw_capability_surfaces = toml_cfg.get("capability_surface_pack_paths", [])
        if isinstance(raw_capability_surfaces, list):
            capability_surface_pack_paths = [
                str(v) for v in raw_capability_surfaces if str(v).strip()
            ]
        elif raw_capability_surfaces:
            capability_surface_pack_paths = [str(raw_capability_surfaces)]
        else:
            capability_surface_pack_paths = []
    hkp_shadow_root = os.environ.get("REINS_HKP_SHADOW_ROOT") or str(
        toml_cfg.get("hkp_shadow_root", "")
    )
    hkp_index_root = os.environ.get("REINS_HKP_INDEX_ROOT") or str(
        toml_cfg.get("hkp_index_root", "")
    )
    hkp_report_root = os.environ.get("REINS_HKP_REPORT_ROOT") or str(
        toml_cfg.get("hkp_report_root", "")
    )
    vault_root = os.environ.get("REINS_VAULT_ROOT") or str(
        toml_cfg.get("vault_root", "")
    )
    ...
    if hkp_bundles_env:
        ...
        if isinstance(raw_hkp_bundles, list):
            ...
            hkp_bundles = []
    intake_paths = {
        "request_intake_state": os.environ.get("REINS_REQUEST_INTAKE_STATE")
        or str(toml_cfg.get("request_intake_state", "")),
        "planning_feed_state": os.environ.get("REINS_PLANNING_FEED_STATE")
        or str(toml_cfg.get("planning_feed_state", "")),
        "p0_incident_state": os.environ.get("REINS_P0_INCIDENT_STATE")
        or str(toml_cfg.get("p0_incident_state", "")),
        "p0_incident_events": os.environ.get("REINS_P0_INCIDENT_EVENTS")
        or str(toml_cfg.get("p0_incident_events", "")),
        "security_signal_state": os.environ.get("REINS_SECURITY_SIGNAL_STATE")
        or str(toml_cfg.get("security_signal_state", "")),
    }
...
if __name__ == "__main__":  # pragma: no cover
    ...
    cfg = instance_config()
    uvicorn.run(
        build_app(cfg["council_root"], cfg["allowlist"], cfg),
        host="127.0.0.1",
        port=cfg["port"],
    )
